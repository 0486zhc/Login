...
class Pager {
	...
	public function __construct($p_totalNum, $p_pageIndex, $p_pageSize = 10, $p_initNum = 3, $p_initMaxNum = 5) {
		...
		$this->_initPagerLegal();     // $p_pageIndex 验证数字的合法性
	}
	...
	private function _getPageUrl() {
		$CurrentUrl = $_SERVER["REQUEST_URI"];          // 当前请求网址
//		echo $CurrentUrl;
//		echo "<br/>";
		$arrUrl = parse_url($CurrentUrl);
//		echo $arrUrl;
		$urlQuery = $arrUrl["query"];							// 获取 参数
		if ($urlQuery) {
...
//			echo $CurrentUrl;
	...
	*/
	private function _initPagerLegal() {
		if ((!is_numeric($this->pageIndex)) || $this->pageIndex < 1) {   // 非数字 或 数字 < 1
			$this->pageIndex = 1;
		}
